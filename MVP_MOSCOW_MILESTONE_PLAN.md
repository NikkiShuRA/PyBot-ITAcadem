...
- **Чт–Пт (26–27 фев):**
  - [ ] Broadcast domain/app/infrastructure + handler + тесты.
  - [ ] ACL/rate-limit + обновление `/help`.
- **Сб (28 фев):**
...
## Acceptance criteria
- [ ] Команда `/broadcast` работает в 3 режимах: `@all`, `role`, `competence`.
- [ ] Не-Admin не может запускать рассылку.
- [ ] Ошибки частичной отправки не валят весь batch, статистика корректна.
- [ ] Auto-bootstrap admin отрабатывает по конфигу.
- [ ] Dev-команды роли не доступны в production-роутерах.
- [ ] Бот и health-check поднимаются в контейнерах.
