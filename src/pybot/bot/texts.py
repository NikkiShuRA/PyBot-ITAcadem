...
POINTS_REASON_LINE = "\nПричина: {reason}"
LEADERBOARD_UNEXPECTED_ERROR = "Не удалось загрузить лидерборд.\nПопробуйте ещё раз позже."
LEADERBOARD_EMPTY = "Пока никто не заработал баллов, будь первым!"
LEADERBOARD_TITLE = "<b>Лидерборд за прошлую неделю</b>"
LEADERBOARD_SECTION_TITLE = "<b>{title}</b>"
LEADERBOARD_ACADEMIC_TITLE = "Академические баллы"
LEADERBOARD_REPUTATION_TITLE = "Репутационные баллы"
...
def render_leaderboard_message(
    *,
    ...
    reputation_block = _render_leaderboard_section(LEADERBOARD_REPUTATION_TITLE, reputation_rows)
    return "\n\n".join(["\n".join(title_lines), academic_block, reputation_block])
...
def _render_leaderboard_section(title: str, rows: Sequence[WeeklyLeaderboardRowDTO]) -> str:
    section_title = LEADERBOARD_SECTION_TITLE.format(title=html.escape(title))
    if not rows:
        return "\n".join([section_title, LEADERBOARD_EMPTY])

    rendered_rows = "\n".join(_render_leaderboard_row(index, row) for index, row in enumerate(rows, start=1))
    return "\n".join([section_title, rendered_rows])


def _render_leaderboard_row(index: int, row: WeeklyLeaderboardRowDTO) -> str:
    place = _leaderboard_place_prefix(index)
    user_link = telegram_user_link(
        row.telegram_id,
        first_name=row.first_name,
        last_name=row.last_name,
        fallback_label="Участник",
    )
    return f"{place} {user_link} - <b>{row.total_points_delta}</b>"


def _leaderboard_place_prefix(index: int) -> str:
    return LEADERBOARD_PLACE_MEDALS.get(index, f"{index}.")
