...
@misc_global_router.message(Command("ping"), flags={"rate_limit": "expensive", "role": "Admin"})
async def cmd_ping(
    message: Message,
    ...
    is_admin = await user_service.check_user_role(user.telegram_id, "Admin")
...
async def give_admin(
    ...
    user = await user_service.set_user_role(user.id, "Admin")
