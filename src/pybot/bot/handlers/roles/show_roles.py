...
@show_roles_global_router.message(
    Command("showroles"),
    flags={"rate_limit": "moderate"},
)
async def handle_show_roles(
    message: Message,
    ...
    try:
        user_roles = await user_roles_service.find_user_roles(target_user.id)
    except UserNotFoundError:
        await message.reply(TARGET_NOT_FOUND)
        return
    except Exception:
        logger.exception("Unexpected error in handle_show_roles")
        await message.reply(ROLE_UNEXPECTED_ERROR)
        return

    if not user_roles:
        await message.reply(user_role_none(target_user.first_name))
        return

    await message.reply(user_role_list(target_user.first_name, user_roles))
