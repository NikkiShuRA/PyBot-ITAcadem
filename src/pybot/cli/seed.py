...
Can be invoked as:
  - ``pybot-seed``            via the installed ``[project.scripts]`` entry point
  - ``python -m pybot.cli.seed``  from the repository root with the venv active
"""
...
def _get_runtime_dependencies() -> RuntimeDependencies:
    ...
    constants_module = import_module("pybot.core.constants")
    db_models_module = import_module("pybot.db.models")
    containers_module = import_module("pybot.di.containers")
    dto_module = import_module("pybot.dto")
    value_objects_module = import_module("pybot.dto.value_objects")
    services_module = import_module("pybot.services")
...
def _get_runtime_logger() -> Logger:
    ...
    logger_module = import_module("pybot.core.logger")
...
def _build_faker(config: FillDatabaseConfig) -> Faker:
    ...
    Faker is a dev-only dependency (:pyproject: ``[dependency-groups].dev``).
    It is imported lazily here so that the module can be imported in production
    ...
    try:
        ...
        raise RuntimeError("faker is required to run the seed script. Install it with: uv sync --dev") from exc
