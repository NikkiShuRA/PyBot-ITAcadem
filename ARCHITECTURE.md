...
## 4. Ограничения реализации и векторы развития (Scalability Trade-offs)
...
* **База Данных (SQLite):**
  * **Ограничение масштабирования (Horizontal Scalability):** Выбор SQLite как базы данных для Production (MVP) блокирует возможность легкого горизонтального масштабирования (запуска нескольких реплик приложения параллельно), так как SQLite предназначена для локального файлового хранения и плохо справляется с параллельной записью в распределенной среде. Если возникнет потребность в нескольких воркерах бота (например, через Kubernetes/Docker Swarm), потребуется миграция на PostgreSQL (инфраструктура к этому уже готова через использование `DATABASE_URL`).
  * **Специфика разработки (Foreign Keys):** Обязательно включать `PRAGMA foreign_keys=ON` при инициализации соединения (в main или DI).
  * **Специфика разработки (Enums):** Используем только строковое хранение (`native_enum=False` в SQLAlchemy).
* **Глобальный Rate Limiting (`aiocache` + `aiolimiter`):**
  * В текущей конфигурации лоджика защиты от спама (Rate Limit Middleware) опирается на связку `aiocache` + инстансы `aiolimiter`. Из-за архитектурных особенностей библиотеки, объект `AsyncLimiter` невозможно закэшировать в Redis, поскольку он содержит асинхронные примитивы и не является сериализуемым.
  * Пока бот работает в режиме моно-реплики (один процесс), использование локального in-memory кэша (`SimpleMemoryCache`) приемлемо. Это рассматривается как архитектурный компромисс. Однако, **в векторе развития на масштабируемость**, при развёртывании нескольких реплик потребуется полный отказ от `aiolimiter` в пользу кастомной реализации логики Token Bucket или Leaky Bucket на стороне Redis (возможно через `redis-py` с Lua-скриптами), чтобы лимиты корректно разделялись между всеми инстансами бота.
