...
def test_import_pybot_core_without_required_env(tmp_path: Path) -> None:
    ...
    import_result = subprocess.run(  # noqa: S603
        [sys.executable, "-c", "from pybot.core import logger; print('ok')"],  # noqa: S607
        capture_output=True,
        check=False,
        cwd=tmp_path,
        env=runtime_env,
    ...
    # Import of pybot.core should not materialize settings anymore.
    ...
    # Explicit settings bootstrap remains fail-fast on missing required env.
