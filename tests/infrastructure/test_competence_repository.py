...
async def test_find_by_names_returns_matching_competencies_case_insensitive(dishka_request_container) -> None:
    ...
    competencies = await repository.find_by_names(db, ["python", "SQL"])

    assert [competence.name for competence in competencies] == ["Python", "SQL"]


@pytest.mark.asyncio
async def test_find_by_names_deduplicates_and_ignores_empty_values(dishka_request_container) -> None:
    db = await dishka_request_container.get(AsyncSession)
    repository = await dishka_request_container.get(CompetenceRepository)
    await create_competence(db, name="Docker")
    await db.commit()

    competencies = await repository.find_by_names(db, [" docker ", "DOCKER", ""])

    assert [competence.name for competence in competencies] == ["Docker"]


@pytest.mark.asyncio
async def test_find_by_names_returns_empty_for_empty_input(dishka_request_container) -> None:
    db = await dishka_request_container.get(AsyncSession)
    repository = await dishka_request_container.get(CompetenceRepository)

    competencies = await repository.find_by_names(db, [])
