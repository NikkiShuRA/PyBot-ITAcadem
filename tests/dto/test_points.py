...
class PointsFactory(ModelFactory[Points]):
    ...
    point_type = lambda: random.choice([PointsTypeEnum.ACADEMIC, PointsTypeEnum.REPUTATION])


@pytest.fixture
def academic_100() -> Points:
    return Points(value=100, point_type=PointsTypeEnum.ACADEMIC)


@pytest.fixture
def reputation_50() -> Points:
    return Points(value=50, point_type=PointsTypeEnum.REPUTATION)
...
def test_points_can_be_created_with_valid_values():
    p = Points(value=500, point_type=PointsTypeEnum.ACADEMIC)
    assert p.value == 500
    assert p.point_type == PointsTypeEnum.ACADEMIC


@pytest.mark.parametrize("value", [-2_147_483_648, 0, 2_147_483_647])
def test_points_accepts_boundary_int32_values(value: int):
    p = Points(value=value, point_type=PointsTypeEnum.REPUTATION)
    assert p.value == value


def test_points_raises_on_too_large_value():
    with pytest.raises(Exception):  # pydantic ValidationError
        Points(value=2_147_483_648, point_type=PointsTypeEnum.ACADEMIC)
...
def test_is_negative(reputation_50: Points, zero_points: Points):
    assert Points(value=-10, point_type=PointsTypeEnum.ACADEMIC).is_negative() is True
    assert zero_points.is_negative() is False


def test_is_negative_delta():
    p = Points(value=100, point_type=PointsTypeEnum.ACADEMIC)
...
def test_compare_to_threshold():
    p = Points(value=150, point_type=PointsTypeEnum.ACADEMIC)
...
def test_compare_to_past_threshold():
    p = Points(value=80, point_type=PointsTypeEnum.REPUTATION)
...
def test_ge_operator_with_int_and_points(academic_100: Points):
    ...
    other = Points(value=80, point_type=PointsTypeEnum.ACADEMIC)
...
def test_lt_operator_with_int_and_points(academic_100: Points):
    ...
    other = Points(value=120, point_type=PointsTypeEnum.ACADEMIC)
    assert academic_100 < other
...
def test_add_with_int(academic_100: Points):
    ...
    assert result.point_type == PointsTypeEnum.ACADEMIC


def test_add_with_same_type_points(reputation_50: Points):
    p2 = Points(value=30, point_type=PointsTypeEnum.REPUTATION)
    result = reputation_50 + p2
    assert result.value == 80
    assert result.point_type == PointsTypeEnum.REPUTATION


def test_add_with_different_type_raises():
    p1 = Points(value=100, point_type=PointsTypeEnum.ACADEMIC)
    p2 = Points(value=50, point_type=PointsTypeEnum.REPUTATION)
...
def test_sub_with_same_type(reputation_50: Points):
    p2 = Points(value=20, point_type=PointsTypeEnum.REPUTATION)
    result = reputation_50 - p2
...
def test_eq_depends_on_value_and_type():
    p1 = Points(value=100, point_type=PointsTypeEnum.ACADEMIC)
    p2 = Points(value=100, point_type=PointsTypeEnum.ACADEMIC)
    p3 = Points(value=100, point_type=PointsTypeEnum.REPUTATION)
    p4 = Points(value=200, point_type=PointsTypeEnum.ACADEMIC)
